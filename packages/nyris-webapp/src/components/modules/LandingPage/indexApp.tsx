...
const LadingPageApp = () => {
  ...
  const getUrlToCanvasFile = (url: string) => {
    dispatch(showResults(""));
    dispatch(loadingActionResults(""));
    return serviceImage(url, settings).then((res) => {
      return dispatch(loadFile(res));
    });
  };

  const onSearchOffersForImage = debounce((r: RectCoords) => {
    ...
    return findByImage(canvas, options, settings).then((res) => {
      return dispatch(loadFileSelectRegion(res));
    });
  }, 1200);
